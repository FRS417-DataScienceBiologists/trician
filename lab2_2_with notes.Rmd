...
```{r}
nrow(mammals) #the number of rows
```

```{r}
ncol(mammals) #the number of columns
```
...
Use | for or instead of & for and. There are no fish that are simultaneously 167 cm and 175 cm.
...
```{r}
filter(fish, !lakeid=="DY", !lakeid=="AL")
```
